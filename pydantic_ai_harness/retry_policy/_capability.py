...
class RetryPolicy(AbstractCapability[AgentDepsT]):
    ...
    def __post_init__(self) -> None:
        ...
        if not math.isfinite(self.max_backoff) or self.max_backoff <= 0:
            raise ValueError(f'max_backoff must be a finite > 0, got {self.max_backoff}')
    ...
    def calculate_delay(self, attempt: int, tool_name: str) -> float:
        ...
        if backoff_factor > 0 and attempt > math.log2(max_backoff) - math.log2(backoff_factor):
            delay = max_backoff
        else:
            delay = math.ldexp(backoff_factor, attempt)
